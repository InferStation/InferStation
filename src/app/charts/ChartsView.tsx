...
const QUANT_ORDER = ["Q8_0", "UD-Q4_K_M", "UD-Q3_K_M", "UD-IQ2_M", "BF16"];
...
const ENGINE_ORDER = ["llama.cpp", "vllm"];
const ENGINE_LABEL: Record<string, string> = {
  vllm: "vLLM",
  "llama.cpp": "llama.cpp",
};

export default function ChartsView({ runs }: { runs: ChartRun[] }) {
  const engines = useMemo<EngineGroup[]>(() => {
    ...
    for (const [engine, mm] of byEngine) {
      groups.push({
        ...
        models: Array.from(mm.values()).sort((a, b) => a.name.localeCompare(b.name)),
      });
    }
  ...
  let selected: ModelGroup | undefined;
  for (const eg of engines) {
    for (const m of eg.models) {
      if (`${eg.engine}::${m.slug}` === selectedKey) selected = m;
    }
  }
  if (!selected && engines[0]) selected = engines[0].models[0];
                  ...
                  return (
                    <li key={key}>
                      ...
                      >
                        ...
                        >
                          {m.params_b}B params · {m.runs.length} runs
                        </div>
                      </button>
                    </li>
                  );
...
function ModelCharts({
  ...
  const devices = useMemo(() => {
    ...
    for (const r of group.runs) {
      const key = `${r.host_slug}::${r.engine_backend}`;
      if (!map.has(key)) {
        const base = vendorColor(r.host_vendor, idx);
        const color = backendShade(base, r.engine_backend);
        map.set(key, {
          key,
  ...
  function lookup(quant: string, deviceKey: string, bs: number): number | null {
    for (const r of group.runs) {
      const key = `${r.host_slug}::${r.engine_backend}`;
      if (
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            {group.name}{" "}
            <span className="text-zinc-500">· {group.params_b}B params</span>
          </h1>
      ...
      {devices.map((d) => (
        <span key={d.key} className="inline-flex items-center gap-1.5">
          ...
          <span>
            ...
            <span className="text-zinc-500">· {d.chip} · {d.backend}</span>
          </span>
        </span>
      ))}
    </div>
