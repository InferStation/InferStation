...
export default function Doc() {
  ...
  badges={[{label:"Dense 8B",tone:"violet"}, {label:"128K ctx"}]}
  links={[{label:"BF16 weights",href:"https://huggingface.co/Qwen/Qwen3-8B",primary:true,external:true}, {label:"GGUF quants",href:"https://huggingface.co/unsloth/Qwen3-8B-GGUF",external:true}]}
  atGlance={[
  ...
  overview={[
    ["Family","Qwen3"],
    ["Repo (BF16)","Qwen/Qwen3-8B"],
    ["Repo (GGUF)","unsloth/Qwen3-8B-GGUF"],
    ["Total params","8B"],
    ["Architecture","Dense decoder, GQA"]
  ]}
  weightsBF16={{repo:"Qwen/Qwen3-8B", note:"Used by vLLM. Download from the public model repository before running the benchmark."}}
  quants={[
...
python3 scripts/bench-batch.py \\
  --filter='<host-profile>:qwen3-8b:<quantization>' \\
  --scope=all --dry-run
