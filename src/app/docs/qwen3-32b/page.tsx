...
export default function Doc() {
  ...
  badges={[{label:"Dense 32B",tone:"violet"}, {label:"128K ctx"}]}
  links={[{label:"BF16 weights",href:"https://huggingface.co/Qwen/Qwen3-32B",primary:true,external:true}, {label:"GGUF quants",href:"https://huggingface.co/unsloth/Qwen3-32B-GGUF",external:true}]}
  atGlance={[
  ...
  overview={[
    ["Family","Qwen3"],
    ["Repo (BF16)","Qwen/Qwen3-32B"],
    ["Repo (GGUF)","unsloth/Qwen3-32B-GGUF"],
    ["Total params","32B"],
    ["Architecture","Dense decoder, GQA"]
  ]}
  weightsBF16={{repo:"Qwen/Qwen3-32B", note:"Used by vLLM. Pre-downloaded on each host into /opt/inferstation/models (Spark) or /home/amd/models (Halo)."}}
  quants={[
    { name:"Q4_K_M", family:"Standard" }
  ]}
  ggufRepo="unsloth/Qwen3-32B-GGUF"
  hosts={["halo", "spark"]}
  engineRows={[
...
ssh spark2 'HB_LLAMA_BIN_DIRS=/home/amd/llama-cuda-bench/llama.cpp/build/bin \\\\
  llama-batched-bench \\\\
    -m /models/Qwen3-32B/Qwen3-32B-Q4_K_M.gguf \\\\
