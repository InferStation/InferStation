...
import HistoryView, { type HistoryRun } from "./HistoryView";
...
export default function HistoryLoader({
  ...
  const [data, setData] = useState<HistoryRun[] | null>(null);
  const [err, setErr] = useState<string | null>(null);

  useEffect(() => {
    fetchAllRuns()
      .then((runs) => {
        const mapped: HistoryRun[] = runs.map((r) => ({
          id: r.id,
  ...
  if (!data) return <p className="p-8 text-sm text-zinc-500">Loading…</p>;
  return <HistoryView navRuns={data} modelSlug={modelSlug} engine={engine} />;
}
