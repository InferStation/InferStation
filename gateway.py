...
async def register_backend(request: Request):
    """Backend self-registration (user only).
    Auth: user API key or session token via Authorization header.
    Backends are always owned by the authenticated user.
    Body: {name, url, models[], client_info?, pricing?, model_map?}
    """
    body = await request.json()
    ...
    if not caller_user:
        raise HTTPException(403, "Valid user API key or session token required")
    name = body["name"]
    url = body["url"].rstrip("/")
    ...
    # Users can only update their own backends
...
async def unregister_backend(request: Request):
    """Remove a backend (user only). Body: {name}. Auth: user API key or session token."""
    body = await request.json()
    authorization = request.headers.get("authorization")
    caller_user = None
    if authorization and authorization.startswith("Bearer "):
    ...
    existing = _find_backend(name)
    if existing:
        if existing.get("owner_id") != caller_user["user_id"]:
