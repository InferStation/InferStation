    ...
    <div className="space-y-16 pb-20">
      ...
      <section className="max-w-4xl mx-auto px-3">
        ...
        <div className="text-center mt-6">
          <Link href="/dashboard/billing" className="inline-flex items-center h-10 px-5 rounded-lg bg-fg text-accent-fg text-sm font-medium hover:bg-fg/90">
      ...
      <section className="max-w-3xl mx-auto px-3">
        ...
        <div className="bg-surface border border-line rounded-xl overflow-hidden">
          <Row label={t({ en: "Provider (GPU owner)", zh: "服务商（GPU 所有者）" })} pct="60%" usd="$0.600" muted={false} />
          <Row label={t({ en: "Platform (Tianshu, ops + R&D)", zh: "平台（天枢，运维 + 研发）" })} pct="25%" usd="$0.250" muted={true} />
          <Row label={t({ en: "Payment channel (Merchant of Record)", zh: "支付渠道（Merchant of Record）" })} pct="15%" usd="$0.150" muted={true} last />
        </div>
        <p className="text-xs text-fg-muted text-center mt-3 leading-relaxed">
          {t({
            en: "The platform fee covers traffic, routing, queueing, billing, fraud control, and ongoing R&D. The payment-channel fee is collected by our Merchant of Record and covers card processing, FX, sales-tax remittance, and refunds.",
            zh: "平台费用于流量、路由、调度、计费、风控与持续研发；渠道费由平台的 Merchant of Record 代为收取，用于信用卡处理、外汇、销售税申报、退款。",
          })}
        </p>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-3">
        ...
        <div className="space-y-3">
          <Faq q={t({ en: "What currency do you charge?", zh: "用什么货币计费？" })} a={t({ en: "USD only. Our payment channel accepts cards in 60+ local currencies and converts to USD at the prevailing rate.", zh: "仅 USD。支付渠道支持30+ 本地货币卡支付，按当时汇率结算为 USD。" })} />
          ...
          <Faq q={t({ en: "Refunds?", zh: "可以退款吗？" })} a={t({ en: "No. All top ups are final and non-refundable once credited to your balance. Your balance never expires, so you can take as long as you need to spend it.", zh: "不退。充值一经到账即为最终状态，不接受退款。但余额永不过期，可以慢慢消费。" })} />
