...
export default function RegisterPage() {
  ...
  const handleSendCode = async () => {
    setError(""); setCodeMsg("")
    if (!EMAIL_RE.test(email.trim())) { setError(t({ en: "Please enter a valid email first", zh: "请先输入有效的邮箱" })); return }
    setSending(true)
    try {
      ...
      setCodeMsg(data?.dev_code ? t({ en: `Verification code generated (dev mode: ${data.dev_code})`, zh: `验证码已生成（开发模式：${data.dev_code}）` }) : t({ en: "Verification code sent — please check your inbox", zh: "验证码已发送，请查收邮箱" }))
      startCooldown(60)
    } catch (err: unknown) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError("")
    if (password !== confirm) { setError(t({ en: "Passwords do not match", zh: "两次密码不一致" })); return }
    if (!checkStrength(password).ok) {
      setError(t({ en: "Password must use at least 8 characters with 3 of: uppercase, lowercase, numbers, symbols.", zh: "密码需包含大写、小写、数字、特殊字符中的至少3种，且不少于8位" }))
      return
    }
    ...
    <div className="min-h-screen grid lg:grid-cols-5 bg-bg">
      ...
      <div className="lg:col-span-3 flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-[22px] font-semibold tracking-tight mb-1">{t({ en: "Create your account", zh: "创建账号" })}</h1>
          <p className="text-sm text-fg-muted mb-6">{t({ en: "Takes less than a minute — no approval needed", zh: "几十秒完成，免审核即可使用" })}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-fg mb-1.5">{t({ en: "Username", zh: "用户名" })}</label>
              <input
                type="text"
                value={username}
            ...
            <div>
              <label className="block text-xs font-medium text-fg mb-1.5">{t({ en: "Email", zh: "邮箱" })}</label>
              <input
                type="email"
                value={email}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  required
                  placeholder={t({ en: "6 digits", zh: "6 位数字" })}
            ...
            >
              {loading ? t({ en: "Signing up...", zh: "注册中..." }) : t({ en: "Sign up", zh: "注册" })}
            </button>
          </form>
          <p className="text-xs text-fg-subtle text-center mt-4 leading-relaxed">
            {t({ en: "By signing up you agree to the", zh: "注册即表示您已阅读并同意" })}
            <Link href="/terms" className="text-fg hover:underline mx-1">{t({ en: "Terms", zh: "服务条款" })}</Link>
            {t({ en: "and", zh: "与" })}
            <Link href="/privacy" className="text-fg hover:underline mx-1">{t({ en: "Privacy Policy", zh: "隐私政策" })}</Link>
          </p>
          <p className="text-center text-[13px] text-fg-muted mt-3">
            {t({ en: "Already have an account? ", zh: "已有账号？" })}<Link href="/login" className="text-fg font-medium hover:underline">{t({ en: "Sign in", zh: "立即登录" })}</Link>
          </p>
        </div>
      </div>
    </div>
